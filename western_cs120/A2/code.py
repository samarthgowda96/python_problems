...
def draw_world(world):
    ...
    # the node should be red (lose) or green (regained)
...
def sim_step(world, p_regain, p_lose):
    # make sure city 0 is always regained
    regain(world, 0)

    # for city in world:
    #     if city[1] == False and numpy.random.rand() < p_regain:
    #         city[1] = True
    #     elif city[1] == False and numpy.random.rand() < p_lose:
    #         random_number = numpy.random.randint()

    for i in range(len(world)):
        # call a number between 0 and 1
        random = numpy.random.rand()
        if world[i][1] == False and random < p_regain:
            regain(world, i)
        if world[i][1] == False and random < p_lose:
            neighbors = world[i][2]
            # grab a random cityno from the list of neighbors
            # it must be greater than or equal to 1 because we can't lose our stronghold
            cityno = numpy.random.randint(1, len(neighbors))
            lose(world, cityno)
...
def time_to_save_world(world, p_regain, p_lose):
    ...
    return regained_world


def save_world_many_times(world, n, p_regain, p_lose):
    times = []
    for i in range(n):
        x = time_to_save_world(world, p_regain, p_lose)
        times.append(x)
    return times
...
if __name__ == "__main__":
    ...
    print(time_to_save_world(my_world, 0.5, 0))
